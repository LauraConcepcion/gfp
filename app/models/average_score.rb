class AverageScore < ActiveRecord::Base
  ...
  def update_quarter_average
    if self.tlresult
      ...
      quarter_averages = AverageScore.where(:profile_id => profile_id, :student_id => student_id, :quarter_id => quarter_id, :teacher_revised => false).where('grade is not null')
      ...
      grades_sum = quarter_averages.inject(0) do |sum, avg|
        tlr_perc = TlresultPercentage.where(:profile_id => profile_id, :tlresult_id => tlresult_id).first
