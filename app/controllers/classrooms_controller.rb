    ...
    csv_rows.each do |row|
      ...
      if row[3].blank?
        ...
        if !student.nil?
          ...
          new_student = Student.create(:name => row[0], :firstsurname => row[1], :secondsurname => row[2], :birthdate => row[4], :phone => row[5], :mail => row[6], :student_code => row[7], :record => row[8], :teacher => current_teacher)
